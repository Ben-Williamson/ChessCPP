    ...
    while (true) {
        ...
        switch (c) {
            case 'P':
                ...
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 0];
                break;
            case 'R':
                whiteRooks |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 1];
                break;
            case 'N':
                whiteKnights |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 2];
                break;
            case 'B':
                whiteBishops |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 3];
                break;
            case 'Q':
                whiteQueens |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 4];
                break;
            case 'K':
                whiteKing |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 5];
                break;
            case 'p':
                blackPawns |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 6];
                break;
            case 'r':
                blackRooks |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 7];
                break;
            case 'n':
                blackKnights |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 8];
                break;
            case 'b':
                blackBishops |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 9];
                break;
            case 'q':
                blackQueens |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 10];
                break;
            case 'k':
                blackKing |= 1ULL << shift_amount;
                zobristHash ^= zobristTables->randomBitstrings[shift_amount * 12 + 11];
                break;
            default:
                break;
        }
...
void PerformantBoard::SetZobristTables(Zobrist* zobrist_tables) {
    zobristTables = zobrist_tables;
}

int PerformantBoard::PieceValues() const {
    int value = 0;
...
bool PerformantBoard::GetWhiteToMove() {
    return whiteToMove;
}
...
void PerformantBoard::MakeMove(Move move) {
    ...
    for (int piece = 0; piece < 12; piece++) {
        Bitboard* bitboard = bitboardPointers[piece];

        if (*bitboard & toMask) {
            // The destination square is occupied in this bitboard.
            // Remove the piece.
            *bitboard &= ~toMask;

            // Need to xor out this piece at the destination square.
            zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + piece];
        }
        else if (*bitboard & fromMask) {
            // The piece being moved is from this bitboard.
            // Remove the piece at fromMask, and add it at toMask.
            *bitboard = ((*bitboard) & ~fromMask) | toMask;

            // Need to xor out the piece on this square,
            zobristHash ^= zobristTables->randomBitstrings[move.from * 12 + piece];
            // then xor in this piece on the new square.
            zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + piece];
        }
    }
    ...
    if (move.promotionPiece) {
        ...
        if (whiteToMove) {
            // xor out the white pawn on the back rank.
            zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 0];

            switch (move.promotionPiece) {
            case 'Q':
                whiteQueens |= toMask;
                zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 4];
                break;
            case 'R':
                whiteRooks |= toMask;
                zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 1];
                break;
            case 'B':
                whiteBishops |= toMask;
                zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 3];
                break;
            case 'N':
                whiteKnights |= toMask;
                zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 2];
                break;
            }
        } else {
            // xor out the black pawn on the back rank.
            zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 6];
            switch (move.promotionPiece) {
            case 'Q':
                blackQueens |= toMask;
                zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 10];
                break;
            case 'R':
                blackRooks |= toMask;
                zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 7];
                break;
            case 'B':
                blackBishops |= toMask;
                zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 9];
                break;
            case 'N':
                blackKnights |= toMask;
                zobristHash ^= zobristTables->randomBitstrings[move.to * 12 + 8];
                break;
            }
        }
    }
    ...
    zobristHash ^= zobristTables->blackToMove;
...
void PerformantBoard::UndoMove() {
    ...
    zobristHash = moveUnmaker.zobrist;
...
uint64_t PerformantBoard::getZobrist() {
    return zobristHash;
}
...
std::ostream& operator<<(std::ostream& os, const PerformantBoard& b)
{
    ...
    for (int i = 7; i >= 0; i--) {
        ...
        if (i == 6) {
            os << "    Zobrist: " << b.zobristHash;
        }
        if (i == 5) {
            os << "    Piece values: " << b.PieceValues();
        }
